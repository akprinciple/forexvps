<?php 
  ...
  if (getallheaders()!=='') {
    $all = getallheaders();
if (isset($all['userId'])&&isset($all['email']) && isset($all['token']) ) {
  $login_date = $all['login_date'];
  $user_check = $all['userId'];
  ...
  $ses_sql = mysqli_query($connect, "SELECT * FROM users WHERE id = '$user_check' && email = '$user_email' && token = '$token'");
  ...
  if ($ses_row['status']=="Unverified") {
    $message = ["status"=> 501, "msg" => "Please verify your account!"];
echo json_encode($message);
  ...
  elseif($session_expiry_date >1){
    $message = ["status"=> 501, "msg" => "Session Expired! Please log in again."];
    echo json_encode($message);
        die();
    }
  elseif ($ses_count < 1) {
    ...
    $message = ["status"=> 501, "msg" => "You don't have access to this page. Contact the administrator for assistance."];
    echo json_encode($message);
        die();
    }
